...
public class TTSGenerator {
 ...
 private static ArmorStandEntity spawnNarratorArmorStand(PlayerEntity player, World world, Item headItem) {
    ...
    armorStand.refreshPositionAndAngles(playerPos.x, playerPos.y + 1.5, playerPos.z, 0, 0);
    armorStand.setCustomName(player.getName());
    armorStand.setCustomNameVisible(true);
    armorStand.setInvisible(false); // Change to true if you want it invisible
    armorStand.setNoGravity(true);    // Let it float
    //armorStand.setNoClip(true);       // Allow free movement through blocks
    ...
    ServerTickEvents.START_SERVER_TICK.register(server -> {
        ...
        Vec3d targetPos = player.getPos().add(0, 1.5, 0); // Target position above the player.
